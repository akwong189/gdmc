...
def test():
    xcenter = STARTX + (ENDX - STARTX) // 2  # getting start + half the length
    ...
    print(heights)
    # print(heights[(0,0)])
    # y = heights[(xcenter, zcenter)]
    print(f"block @a {STARTX} {y} {STARTZ}")
    # INTF.placeBlock(xcenter, y, zcenter, "emerald_block")
    # INTF.placeBlock(xcenter, y+1, zcenter, "emerald_block")
    # INTF.placeBlock(xcenter, y+2, zcenter, "emerald_block")
    # INTF.runCommand(f"tp @a {xcenter} {y+5} {zcenter}")


    # GEO.placeLine(xcenter, y, zcenter, xcenter, y+5, zcenter, "granite")
