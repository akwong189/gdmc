...
for x in range(x_max):
    for z in range(z_max):
        ...
        if location.shape == (h_x, h_z) and location.max() == location.min() and mask.max() == 0 and mask.min() == 0:
            print(f"Building house at {x},{location.min()},{z}")
            # INTF.placeBlock(x, location.min(), z, "cobblestone")
            # INTF.placeBlock(x, location.min(), z+h_z, "cobblestone")
            # INTF.placeBlock(x+h_x, location.min(), z, "cobblestone")
            # INTF.placeBlock(x+h_x, location.min(), z+h_z, "cobblestone")
            house.place_structure(INTF, x, location.min(), z, NBTBuildings.Cardinals.NORTH)
        #     x += h_x
        #     z += h_z
